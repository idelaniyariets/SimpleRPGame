...
func _on_area_2d_body_entered(body):
	#катсцена на локации 1. триггер - вход в область
	if body.name == "Player":
		$CanvasLayer/AnimatedSprite2D2.visible = true #начало диалога
		...
		get_tree().create_tween().parallel().tween_property($CanvasLayer/Label, "text", "", 1)
		get_tree().create_tween().parallel().tween_property($CanvasLayer/Label, "modulate", Color8(255, 255, 255, 0), 1)
		$CanvasLayer/AnimatedSprite2D2.visible = false #конец
		print(self.position)
		$Area2D/CollisionShape2D.disabled = true
		$CanvasLayer/Label.text = ""
		$AnimatedSprite2D.play("walk") #анимация движения npc
