...
#соотнесение состояний, движение
	if state == IDLE:
		idle_state()
...
func move_state():
	#основное состояние
	#перемещение по случайно выбранным координатам
	#выполнение действий на координатах
	anim.play("MOVE")
	randomize()
...
func attack_state():
	#аттака активирует / деактивирует коллизии
...
func summon_state():
	#призыв 2х грибов для атаки игрока
	await get_tree().create_timer(1).timeout
...
func death_state():
	#состояние смерти. уничтожение призванных грибов.
	#анимация. предсмертный диалог.
	$"../summoned".queue_free()
...
func wait_state():
	#состояние ожидания для нанесения урона
	$Timer.start()
	anim.play("IDLE")

func idle_hit_state():
	#анимация во время ожидания
	anim.play("IDLE")

func arrival_state():
	#анимация появлния. вступительный диалог
...
func _on_mobe_health_damage_recived():
	#изменение состояния по сигналу
	state = HIT

func _on_mobe_health_no_hp():
	#ищменение состояния по сигналу
	dead = true
	state = DEATH
...
func _on_area_2d_8_body_entered(body: Node2D) -> void:
	#смена состояния по тригеру
	state = ARRIVAL

func _on_hit_box_area_entered(area: Area2D) -> void:
	#отправка сигнала об атаке
