...
func _physics_process(delta: float) -> void:
	#функция обновляющая позицию, сравнивающая состояния
	#запуск движения
...
func move_state():
	#состояние передвижения: выбирается случайная позиция
	#по случайным координатам перемещается по попзициям и делает определенные действия
	anim.play("MOVE")
	randomize()
...
func hit_state():
	#находитяся в ожидании, чтобы игрок мог нанести урон
	anim.play("HIT")
	await anim.animation_finished
	state = IDLE_HIT
...
func wait_state():
	#ожидние чтобы игрок мог наносить урон
	$Timer.start()
	anim.play("IDLE")

func idle_hit_state():
	#проигрывает анимацию во время ожидания
	anim.play("IDLE")

func _on_hit_box_area_entered(_area):
	#отправляет сигнал, когда область атаки задевает игрока
